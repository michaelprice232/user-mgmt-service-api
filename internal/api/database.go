...
func (m *UserModel) queryUsers(offset, limit int, nameFilter string) ([]User, error) {
	...
	if nameFilter != "" {
		rows, err = m.DB.Query(`SELECT full_name, email FROM users WHERE full_name like '%' || $1 || '%' ORDER BY user_id OFFSET $2 LIMIT $3`, nameFilter, offset, limit)
	} else {
		rows, err = m.DB.Query(`SELECT full_name, email FROM users ORDER BY user_id OFFSET $1 LIMIT $2`, offset, limit)
	}
	if err != nil {
